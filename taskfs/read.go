...
func NewTaskDir(dirAbsPath string) (TaskDirReader, error) {
	...
	if err != nil {
		...
		return TaskDirReader{}, errwrap.Wrap(msg, err)
	}
	dir := TaskDirReader{
...
func (dir *TaskDirReader) readAllPathsInDir() error {
	...
	if err != nil {
		return errwrap.Wrap("walk directory", err)
	}
	return nil
}
...
func (dir TaskDirReader) ReadFile(relPath string) ([]byte, error) {
	...
	if strings.Contains(filePathRel, "..") {
		...
		return nil, errwrap.Trace(errors.New(msg))
	}
...
func (dir TaskDirReader) Toml() (TaskToml, error) {
	...
	if err != nil {
		return TaskToml{}, errwrap.Trace(err)
	}
	taskToml := TaskToml{}
	...
	if err != nil {
		...
		if errors.As(err, &details) {
			msg := details.String()
			return TaskToml{}, errwrap.Wrap(msg, err)
		}
		return TaskToml{}, errwrap.Trace(err)
	}
	return taskToml, nil
...
func (dir TaskDirReader) Testing() (Testing, error) {
	...
	if err != nil {
		return Testing{}, errwrap.Trace(err)
	}
	t := Testing{
	...
	if taskToml.Testing.Type == "checker" {
		...
		if checker == "" {
			...
			return Testing{}, errwrap.Error(msg)
		}
		t.Checker = checker
	}
...
func (dir TaskDirReader) Metadata() (Metadata, error) {
	taskToml, err := dir.Toml()
	if err != nil {
		return Metadata{}, errwrap.Trace(err)
	}
...
func (dir TaskDirReader) Solutions() ([]Solution, error) {
	taskToml, err := dir.Toml()
	if err != nil {
		return []Solution{}, errwrap.Trace(err)
	}
...
func (dir TaskDirReader) Stories() (I18N[StoryMd], error) {
	...
	if err != nil {
		...
		return I18N[StoryMd]{}, errwrap.Wrap(msg, err)
	}
...
func ParseMdStory(content string, lang string) (StoryMd, error) {
	...
	for _, translation := range mdStorySectionI18n {
		...
		if err != nil {
			...
			return StoryMd{}, errwrap.Wrap(msg, err)
		}
		if len(parsed) > 0 {
			if foundLang {
				msg := "story section dividers in multiple langs"
				return StoryMd{}, errwrap.Error(msg)
			}
			foundLang = true
		}
...
func (dir TaskDirReader) Statement() (Statement, error) {
	...
	if err != nil {
		msg := "read task.toml"
		return Statement{}, errwrap.Wrap(msg, err)
	}
...
func (dir TaskDirReader) Example(index int) (Example, error) {
	...
	if err != nil && !errors.Is(err, os.ErrNotExist) {
		...
		return Example{}, errwrap.Wrap(msg, err)
	}
...
func (dir TaskDirReader) Examples() ([]Example, error) {
	...
	for _, file := range files {
		...
		if err != nil {
			...
			return []Example{}, errwrap.Error(msg)
		}
		idxMap[index] = true
	}
	...
	if err != nil {
		return []Example{}, errwrap.Error(err.Error())
	}
...
func (dir TaskDirReader) TestGroups() ([]TestGroup, error) {
	...
	if err != nil {
		...
		return []TestGroup{}, errwrap.Wrap(msg, err)
	}
...
func parseTestGroupLine(idx int, line string) (tg TestGroup, err error) {
	...
	if len(matches) != 1 || len(matches[0]) < 1 {
		...
		return TestGroup{}, errwrap.Error(msg)
	}
...
func (dir TaskDirReader) Scoring(noOfTests int, subtasks []Subtask) (Scoring, error) {
	...
	if err != nil {
		msg := "read task.toml"
		return Scoring{}, errwrap.Wrap(msg, err)
	}
...
func (dir TaskDirReader) Archive() (Archive, error) {
	...
	if err != nil {
		...
		return Archive{}, errwrap.Wrap(msg, err)
	}
	archive := Archive{}
	for _, file := range files {
