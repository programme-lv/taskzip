...
func TestClientErrorWrapping(t *testing.T) {
	err := errwrap.ClientError("client supplied bad request")
	err2 := errwrap.AddTrace(err)
	err3 := errwrap.ServerError("test", err2)
	err4 := errwrap.AddTrace(err3)
	err5 := errwrap.ServerError("test", err4)
	err6 := errwrap.AddTrace(err5)
	err7 := errwrap.ServerError("test", err6)
	err8 := errwrap.AddTrace(err7)
	err9 := errwrap.ServerError("server error", fmt.Errorf("some error"))
